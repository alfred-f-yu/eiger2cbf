...
int main(int argc, char **argv) {
    ...
    H5LTread_dataset_string(hdf, "/entry/instrument/detector/detectorSpecific/data_collection_date", collection_time);
    ...
    for (frame = from; frame <= to; frame++) {
        ...
        ret = H5Sselect_hyperslab(memspace, H5S_SELECT_SET, offset_out, NULL, count, NULL);
        if (ret < 0) {
